...
def decay(orin_data,mode_num,node_num,start_t,end_t,max_min_res):
    new_data = orin_data.copy()
    for i in range(start_t, end_t):
        new_data[mode_num][node_num][i] = new_data[mode_num][node_num][i - 1] - \
                                          (max_min_res[0][mode_num] - max_min_res[1][mode_num]) / 35
    # print("d#",new_data[mode_num][node_num][start_t:end_t])
    return new_data

def increase(orin_data,mode_num,node_num,start_t,end_t,max_min_res):
    new_data = orin_data.copy()
    for i in range(start_t, end_t):
        new_data[mode_num][node_num][i] = new_data[mode_num][node_num][i - 1] + \
                                          (max_min_res[0][mode_num] - max_min_res[1][mode_num]) / 35
...
def internode_anomaly(adj_maxt , series_data, inject_mode_num,inject_node_num, start_time, end_time):
    ...
    if target > 0.3 and 0.6 < correlation[0][1] <= 1:
        ...
        if trend < 0:
            ...
            inj_type = 3
        return inj_type
    elif target > 0.3 and -0.6 > correlation[0][1] >= -1:
